-- Criar tabela de perfis profissionais do usuário
CREATE TABLE IF NOT EXISTS user_profiles (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  profile_type TEXT NOT NULL CHECK (profile_type IN ('pintor', 'preparador', 'polidor', 'lanterneiro', 'dono-oficina')),
  created_at TIMESTAMP WITH TIME ZONE DEFAULT timezone('utc'::text, now()) NOT NULL,
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT timezone('utc'::text, now()) NOT NULL,
  UNIQUE(user_id, profile_type)
);

-- Criar tabela de anotações por perfil
...
CREATE INDEX idx_profile_notes_user_id ON profile_notes(user_id);
CREATE INDEX idx_profile_services_profile_id ON profile_services(profile_id);
CREATE INDEX idx_profile_services_user_id ON profile_services(user_id);
CREATE INDEX idx_profile_expenses_profile_id ON profile_expenses(profile_id);
CREATE INDEX idx_profile_expenses_user_id ON profile_expenses(user_id);
