...
export function SwipeableServiceItem({
  ...
  const carregarCategorias = () => {
    const data = getProfileNoteData(servico.id, NOTA_KEY);
  ...
  useEffect(() => {
    ...
    // Re-carrega quando outra aba atualiza o localStorage
    const handleStorage = () => carregarCategorias();
    window.addEventListener('storage', handleStorage);

    // Re-carrega quando o usuário volta para a aba/janela
    window.addEventListener('focus', handleStorage);

    // Re-carrega quando categorias são salvas na mesma aba
    const handleCustom = (e: Event) => {
      const detail = (e as CustomEvent).detail;
      if (!detail?.servicoId || detail.servicoId === servico.id) {
        carregarCategorias();
      }
    };
    window.addEventListener('profileNotesUpdated', handleCustom);

    return () => {
      window.removeEventListener('storage', handleStorage);
      window.removeEventListener('focus', handleStorage);
      window.removeEventListener('profileNotesUpdated', handleCustom);
    };
          ...
          <div className="p-4 hover:bg-gray-50 transition-colors">
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                {/* Categorias como subtítulo — máx 2 visíveis + contador */}
                {categorias.length > 0 ? (
                  <div className="flex items-center gap-1 mt-0.5 overflow-hidden">
                    {categorias.slice(0, 2).map(cat => (
                      <span key={cat} className="inline-flex items-center px-2 py-0.5 bg-indigo-100 text-indigo-700 text-xs rounded-full font-medium whitespace-nowrap max-w-[90px] truncate">
                        {cat}
                      </span>
                    ))}
                    {categorias.length > 2 && (
                  ...
                  <p className="text-sm text-gray-400 truncate italic">Sem categoria</p>
                )}
              </div>
